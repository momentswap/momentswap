...
contract MomentSwapFRC721 is ERC721URIStorage {
    ...
    event NewMomentSwapFRC721NFTMinted(
        address indexed sender, uint256 indexed tokenId, uint256 indexed timestamp, string tokenURI
    );
    ...
    function mintMomentSwapNFT(address owner, string memory ipfsURI) public returns (uint256) {
        ...
        momentSwapFRC721NFT memory newNFT =
            momentSwapFRC721NFT({owner: msg.sender, tokenURI: ipfsURI, tokenId: newItemId, timestamp: block.timestamp});
    ...
    /**
     */
    function mintMultipleMomentSwapNFTs(address owner, string[] memory ipfsMetadata)
        public
