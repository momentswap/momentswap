...
task("callRPC", "callsWallabyRPC")
  ...
  .addOptionalParam("rpcUrl", "rpc url (wallaby default)", defaultRPC, types.string)
...
task("formatData", "create json data from contract deploy")
  ...
  .addOptionalParam("explorerUrl", "block explorer base url", defaultExplorerUrl, types.string)
  .addOptionalParam("networkName", "network common name", defaultNetworkName, types.string)
  .setAction(async ({ data, contract, networkName, explorerUrl }: TaskArguments) => {
    const jsonObj = {
      contract,
      network: networkName,
...
task("logToFile", "writes outputs to a file")
  ...
  .setAction(async ({ filePath, data, contract }: TaskArguments, hre) => {
    ...
    await appendFile(path.resolve(__dirname, filePath), `${JSON.stringify(json)}\n`, "utf-8");
  });

task("getNetworkInfo", "returns network object").setAction(async (taskArgs: TaskArguments, hre) => {
