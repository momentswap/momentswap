task("deploy", "Space FNS Use Case").setAction(async () => {
  // Create main wallet
...
task("listItem", "Space FNS Use Case")
  ...
  .setAction(async ({ fnscontract, marketcontract }) => {
    ...
    const registerResult = await spaceFNSContractA.register(mainDomainName);
...
task("lendItem", "Space FNS Use Case")
  ...
  .setAction(async ({ fnscontract, marketcontract }) => {
    ...
    const walletB = new ethers.Wallet(
      "0xd5f021220a901173d4f48faf5a1ac5d9e19bc74c6aaf8f3f4e46d530fba1bab0",
      ethers.provider,
    );
    ...
    const getListingResult = await spaceMarketContractB.getListing(fnscontract, 3);
...
task("getDomain", "Space FNS Use Case")
  .addParam("fnscontract", "Address of SpaceFNS contract")
  .addParam("marketcontract", "Address of RentMarket contract")
  .setAction(async ({ fnscontract, marketcontract }) => {
    // Create main wallet
    ...
    // console.log(`🎠 spaceFNSContractB.register(ironman)`);
    // const registerResult = await spaceFNSContractB.register("ironman")
    // console.log("🏵️ ", registerResult);

    console.log(`🎠 spaceFNSContractB.getMainDomainAndChild(${walletB.address})`);
    const getMainDomainAndChildResult = await spaceFNSContractA.getMainDomainAndChild(
      "0xBaAcA1B51B2A52CB1300046f0d310f08fBc7f09d",
    );
    console.log("🏵️ ", getMainDomainAndChildResult);
  });
